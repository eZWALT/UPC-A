...
int main() {
    srand(time(NULL));
    //Leemos variables
    unsigned int soupSize, dicSize, insertedSize, wordsMaxSize;
    ...
    for (unsigned int i = 0 ; i < insertedSize; ++i) {
    ...
    for (unsigned int i = 0; i < soupSize; ++i) {
        for (unsigned int j = 0; j < soupSize; ++j) {
            outDoc << sopa[i][j] << " ";
        }
        outDoc << endl;
    }
